...
class JustTheTooltip extends StatefulWidget implements JustTheInterface {
  ...
  @override
  JustTheTooltipState<OverlayEntry> createState() => _JustTheTooltipOverlayState();
}
...
abstract class JustTheTooltipState<T> extends State<JustTheInterface> with SingleTickerProviderStateMixin {
  T? get entry;
  ...
  Widget build(BuildContext context) {
    ...
    if (triggerMode == TooltipTriggerMode.manual) {
      ...
      if (_mouseIsConnected || widget.openTooltipOnHover) {
        result = MouseRegion(
