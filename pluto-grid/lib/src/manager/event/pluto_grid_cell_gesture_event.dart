...
class PlutoGridCellGestureEvent extends PlutoGridEvent {
  ...
  void handler(PlutoGridStateManager stateManager) {
    switch (gestureType) {
      ...
      case PlutoGridGestureType.onSecondaryTap:
        ...
        break;
      case PlutoGridGestureType.none:
        _onNone(stateManager);
        break;
      default:
    }
  }
  ...
  void _onTapUp(PlutoGridStateManager stateManager) {
    if (_setKeepFocusAndCurrentCell(stateManager)) {
      ...
      // Multiple selection with holding CTRL
      if (isControlPressed) {
    ...
    if (stateManager.isCurrentCell(cell) && stateManager.isEditing != true) {
      ...
      if (isControlPressed) {
        ...
        stateManager.currentSelectedCells.clear();
        stateManager.setCurrentCell(cell, rowIdx);
      }
    }
  }

  void _multiCellSelect(PlutoGridStateManager stateManager) {
    PlutoGridSelectingCellPosition? selectedCell = PlutoGridSelectingCellPosition(field: column.field, rowIdx: rowIdx);
    if (selectedCell != null && !stateManager.currentSelectedCells.contains(selectedCell))
      stateManager.currentSelectedCells.add(selectedCell);
    else
      stateManager.currentSelectedCells.remove(selectedCell);
    stateManager.currentSelectingPositionList.clear();
    stateManager.currentSelectingPositionList.addAll(stateManager.currentSelectedCells);
    // If the user started selecting without holding ctrl button this is to remove the activated row, so that the user sees only the selected cells instead
    if (stateManager.currentRowIdx != null) {
      stateManager.currentSelectedCells.add(PlutoGridSelectingCellPosition(
  ...
  void _onNone(PlutoGridStateManager stateManager) {
    if (_setKeepFocusAndCurrentCell(stateManager)) {
      return;
    }
    ...
    if (stateManager.isCurrentCell(cell) && stateManager.isEditing != true) {
      ...
      stateManager.setColorCell(cell, rowIdx);
    }
  }
...
enum PlutoGridGestureType {
  ...
  bool get isNone => this == PlutoGridGestureType.none;
}
