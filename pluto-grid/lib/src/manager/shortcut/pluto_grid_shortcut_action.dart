...
class PlutoGridActionDefaultTab extends PlutoGridShortcutAction {
  ...
  void execute({
    ...
    keyEvent.event.isShiftPressed ? _moveCellPrevious(stateManager) : _moveCellNext(stateManager);
...
/// Select multiple cells with CTRL
class PlutoGridActionSelectMultiple extends PlutoGridShortcutAction {
  const PlutoGridActionSelectMultiple();
  ...
  void execute({
    ...
    PlutoGridSelectingCellPosition? selectedCell = stateManager.setCellsCurrentSelecting();
    if (selectedCell != null && !stateManager.currentSelectingPositionList.contains(selectedCell))
      stateManager.currentSelectingPositionList.add(selectedCell);
  }
}
