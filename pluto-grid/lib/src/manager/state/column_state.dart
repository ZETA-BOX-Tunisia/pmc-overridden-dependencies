...
mixin ColumnState implements IPlutoGridState {
  ...
  void sortAscending(PlutoColumn column, {bool notify = true}) {
    ...
    var dateFormat = intl.DateFormat('dd.MM.yyyy');
    DateTime? date;
    try {
      date = dateFormat.parseLoose(refRows.map((element) => element.cells[column.field]!.valueForSorting).first);
    } catch (_) {
      date = null;
    }
    if (date != null) {
      ...
      refRows.sort((a, b) {
        ...
        try {
          result = dateFormat
              .parseLoose(a.cells[column.field]!.valueForSorting)!
              .compareTo(dateFormat.parseLoose(b.cells[column.field]!.valueForSorting)!);
          return result;
        } catch (_) {
          return 0;
        }
      ...
      // Other types sorting
      compare(a, b) {
        return a.cells[column.field]!.valueForSorting is String
            ? column.type.compare(
                (a.cells[column.field]!.valueForSorting as String).toLowerCase(),
                (b.cells[column.field]!.valueForSorting as String).toLowerCase(),
              )
            : column.type.compare(
  ...
  void sortDescending(PlutoColumn column, {bool notify = true}) {
    ...
    if (date != null) {
      ...
      refRows.sort((a, b) {
        ...
        try {
          result = dateFormat
              .parseLoose(b.cells[column.field]!.valueForSorting)!
              .compareTo(dateFormat.parseLoose(a.cells[column.field]!.valueForSorting)!);
          return result;
        } catch (_) {
          return 0;
        }
