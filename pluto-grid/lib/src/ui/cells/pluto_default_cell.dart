...
class _PlutoDefaultCellState extends PlutoStateWithChange<PlutoDefaultCell> {
  ...
  String get _groupCount => _compactCount ? stateManager.rowGroupDelegate!.compactNumber(widget.row.type.group.children.length) : widget.row.type.group.children.length.toString();
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      ...
      if (_showGroupCount)
        Text(
          '($_groupCount)',
          style: stateManager.configuration.style.cellTextStyle.copyWith(decoration: TextDecoration.none, fontWeight: FontWeight.normal, color: widget.row.checked! ? const Color(0xFFFFFFFF) : const Color(0xFF4F4F4F)),
        ),
    ]);
  }
}
...
class CheckboxSelectionWidgetState extends PlutoStateWithChange<CheckboxSelectionWidget> {
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.row.checked! || widget.cell.overrideColorCheckbox == null) {
      ...
      return Container(
        ...
        color: widget.cell.overrideColorCheckbox,
        child: Center(
          child: Container(
            width: 13,
            height: 13,
...
class _DefaultCellWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text(
      _text,
      style: stateManager.configuration.style.cellTextStyle.copyWith(decoration: TextDecoration.none, fontWeight: FontWeight.normal, color: row.checked! ? const Color(0xFFFFFFFF) : const Color(0xFF4F4F4F)),
